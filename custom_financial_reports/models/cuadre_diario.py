...
class ReportCuadreDiario(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # Función auxiliar para convertir a USD usando sudo()
        ...
        domain_fac = [
            ...
            ('state', '=', 'posted'),           # Solo en estado Publicado/Confirmado
        ]
        ...
        # Aplicar filtro por usuario creador si fue seleccionado en el wizard
        if user_id:
            domain_fac.append(('create_uid', '=', user_id.id))
            domain_pay.append(('create_uid', '=', user_id.id))

        # 1. Búsqueda con sudo() filtrando únicamente facturas confirmadas
        fac_list = self.env['account.move'].sudo().search(domain_fac, order='name asc')

        # Variables para mantener compatibilidad con la vista QWeb y diccionario de retorno
        ...
        # 2. Búsqueda de Pagos con sudo() para desglosar cada pago individualmente
        ...
        # Detalle de cada transacción bancaria/tarjeta por separado
        card_bank_details = []
        for p in (tarjeta_payments | banco_payments):
            ...
            # sudo() aplicado a las relaciones de conciliación
