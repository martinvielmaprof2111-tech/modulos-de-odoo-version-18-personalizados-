...
# Lista global de diarios permitidos por sucursal
ALLOWED_JOURNALS = [
...
class ReportCuadreDiarioDetalles(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # Búsqueda con sudo() filtrando exclusivamente por los diarios autorizados
        invoices = self.env['account.move'].sudo().search([
            ('invoice_date', '>=', date_from),
            ('invoice_date', '<=', date_to),
            ('company_id', '=', company.id),
            ('journal_id.name', 'in', ALLOWED_JOURNALS),  # <-- FILTRO DE DIARIO AGREGADO
        ...
        for inv in invoices:
            ...
            if not is_cancel:
                ...
                for pay in matched_payments:
                    ...
                    # REGLA PARA PAGOS EN DÓLARES (Zelle, Efectivo USD)
                    if pay.currency_id == currency_usd:
                        rate = pay.currency_id._get_conversion_rate(
                            currency_usd, company.currency_id, company, pay.date or fields.Date.today()
                        )
                        
                        if not rate and inv.amount_total and inv.currency_id != currency_usd:
                            inv_usd = get_amount_usd(inv.amount_total, inv.currency_id, inv.invoice_date)
                            if inv_usd:
                                rate = inv.amount_total / inv_usd
                        
                        pay_usd = pay.amount
                        pay_amount = pay.amount * rate if rate else pay.amount
