...
# Lista global de diarios autorizados por sucursal
ALLOWED_JOURNALS = [
...
class ReportResumenLibroVentas(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        # Búsqueda de facturas y notas de crédito publicadas filtradas por diarios permitidos
        invoices = self.env['account.move'].sudo().search([
            ('invoice_date', '>=', docs.date_from),
            ('invoice_date', '<=', docs.date_to),
            ('company_id', '=', company.id),
            ('journal_id.name', 'in', ALLOWED_JOURNALS),  # <-- FILTRO DE DIARIOS PERMITIDOS
