...
from ..utils.data_exporter import IcomprasDataExporter
    ...
    def action_test_configuration(self):
        ...
        for product in products:
            try:
                ...
                _logger.info(f"[iCompras360 LOG] Producto ID {product.id} -> {linea_traza}")
            except Exception as e:
                _logger.error(f"[iCompras360 LOG] Error procesando Producto ID {product.id}: {str(e)}")
